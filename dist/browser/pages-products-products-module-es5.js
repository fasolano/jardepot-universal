...
var ProductZoomComponent = /** @class */ (function () {
    ...
    ProductZoomComponent.ctorParameters = function () { return [
...
var ProductComponent = /** @class */ (function () {
    function ProductComponent(appService, activatedRoute, dialog, formBuilder, cd, router, title, meta) {
    ...
    ProductComponent.prototype.ngOnInit = function () {
        ...
        this.sub = this.activatedRoute.params.subscribe(function (params) {
            ...
            _this.getProductByName(params.product);
            _this.getRelatedProducts(params.product);
            _this.productName = params.product;
            // this.scrollToTop();
        });
    ...
    ProductComponent.prototype.onWindowResize = function () {
    ...
    ProductComponent.prototype.getRelatedProducts = function (product) {
        var _this = this;
        this.appService.getProductsRelated(product).subscribe(function (data) {
            _this.relatedProducts = data;
            _this.refreshPage();
        });
    };
    ProductComponent.prototype.getProductByName = function (product) {
        var _this = this;
        this.appService.getProductByName(product).subscribe(function (data) {
            // @ts-ignore
            if (data == 0) {
                _this.router.navigate(['/busqueda/no-encontrado']);
                return;
            }
            _this.product = data;
            _this.json = {
                ...
                'mpn': _this.product.mpn,
                'brand': _this.product.brand,
                'image': data.images[0].medium,
                'description': _this.product.description,
                'offers': {
                    '@type': 'Offer',
                    'price': _this.product.newPrice
                }
            };
            _this.dataSheet = _this.product.dataSheet;
            _this.image = data.images[0].medium;
            _this.zoomImage = data.images[0].big;
            _this.meta.updateTag({ name: 'description', content: _this.product.metaDescription.substr(0, 150) });
            _this.meta.updateTag({ name: 'keywords', content: _this.product.keywords });
            _this.title.setTitle(_this.product.metaTitle.substr(0, 70));
            _this.refreshPage();
        });
    };
    ProductComponent.prototype.refreshPage = function () {
        var _this = this;
        setTimeout(function () {
        ...
        if (this.directiveRef.swiper()) {
            setTimeout(function () {
                _this.directiveRef.swiper().lazy.load();
            }, 0);
        }
    };
    ...
    ProductComponent.prototype.onSubmit = function (values) {
        var _this = this;
        if (this.form.valid) {
            this.appService.enviarBusqueda(values, '').subscribe(function (data) {
                var res = JSON.parse(JSON.stringify(data));
                _this.sendComment = res;
                _this.form.reset();
                _this.createForm();
            });
        }
    ...
    ProductComponent.prototype.handleImgError = function (ev) {
        var source = ev.srcElement;
        var imgSrc = "assets/images/productos/generico2.jpg";
        source.src = imgSrc;
    };
    ProductComponent.prototype.openDeliveryTermsDialog = function () {
        var text = 'Los envíos gratuitos que ofrece JarDepot son a la cobertura terrestre normal de las paqueterías con las que tenemos convenio (ODM).<br>' +
        ...
        var dialogRef = this.dialog.open(_shared_dialog_dialog_component__WEBPACK_IMPORTED_MODULE_10__["DialogComponent"], {
    ...
    ProductComponent.prototype.scrollToTop = function () {
        var _this = this;
        if (this.window) {
    ...
    ProductComponent.prototype.getDistributions = function () {
...
var ProductsComponent = /** @class */ (function () {
    function ProductsComponent(appSettings, activatedRoute, appService, dialog, router, formBuilder, meta, title, snackBar) {
    ...
    ProductsComponent.prototype.ngOnInit = function () {
        ...
        this.sub = this.activatedRoute.params.subscribe(function (params) {
            _this.moveToFirstPage();
            _this.activeFilters['brand'] = [];
            _this.activeFilters['characteristic'] = [];
            _this.activeFilters['range'] = [];
            _this.descriptionNivel2 = '';
            if (params['nivel1'] !== undefined && params['nivel2'] !== undefined) {
                _this.nivel1 = params['nivel1'];
                _this.nivel2 = params['nivel2'];
                _this.busqueda = true;
                _this.nivel1 = _this.nivel1.replace(/-/g, ' ');
                _this.nivel2 = _this.nivel2.replace(/-/g, ' ');
                _this.setTypeProductFilter(_this.nivel1);
                _this.filters(_this.nivel2);
                _this.getProducts();
                _this.getSectionsProducts(_this.nivel1, _this.nivel2);
                _this.appService.getDescriptionNivel2(_this.nivel1, _this.nivel2).subscribe(function (data) {
                    var res = JSON.parse(JSON.stringify(data));
                    _this.descriptionNivel2 = res.result.texto;
                ...
                _this.titleProducts = _this.nivel2;
            }
            else if (params['search'] !== undefined) {
                _this.busqueda = false;
                _this.busquedaEmpty = false;
                _this.appService.getProductsSearch(params['search']).subscribe(function (data) {
                    _this.textSearch = params['search'];
                    _this.orderByOption = 'default';
                    // @ts-ignore
                    if (data === 'emptyProducts') {
                        _this.busquedaEmpty = true;
                        _this.products = [];
                    }
                    else {
                        _this.products = data;
                    }
                    _this.titleProducts = 'Buscaste: ' + _this.textSearch;
                    _this.meta.updateTag({ name: 'description', content: 'Busca los productos que necesites Jardepot' });
                    _this.meta.updateTag({ name: 'keywords', content: 'Busca los productos que necesites Jardepot' });
                    _this.title.setTitle('Jardepot, el lugar donde encuentras todo lo que necesitas');
                });
            }
            else {
                _this.busqueda = false;
                _this.appService.getProductsOffer().subscribe(function (data) {
                    _this.meta.updateTag({ name: 'description', content: 'Busca los productos con las mejores ofertas en Jardepot' });
                    _this.meta.updateTag({ name: 'keywords', content: 'Ofertas Jardepot' });
                    _this.title.setTitle('Jardepot el lugar donde encuentras las mejores ofertas');
                    _this.orderByOption = 'relevant';
                    // @ts-ignore
                    if (data === 'emptyProducts') {
                        _this.busquedaEmpty = false;
                        _this.products = [];
                        _this.titleProducts = 'Sin productos en oferta: ';
                    }
                    else {
                        _this.products = data;
                        _this.titleProducts = 'Productos en oferta: ';
                    }
                });
            }
        });
    ...
    ProductsComponent.prototype.handleImgError = function (ev) {
        var source = ev.srcElement;
        var imgSrc = "assets/images/productos/generico2.jpg";
        source.src = imgSrc;
    };
    ProductsComponent.prototype.setTypeProductFilter = function ($nivel1) {
    ...
    ProductsComponent.prototype.moveToFirstPage = function () {
        this.page = 1;
        return;
    };
    ProductsComponent.prototype.addCategoriteFilter = function ($section, $event) {
        if (this.activeFilters['brand'].length == 0 || this.activeFilters['brand'].length == this.sections.length) {
            ...
            var existCategory = this.activeFilters['brand'].filter(function (section) { return section == $section; });
            if (existCategory.length > 0) {
                this.activeFilters['brand'] = this.activeFilters['brand'].filter(function (section) { return section != $section; });
    ...
    ProductsComponent.prototype.addCharacteristicFilter = function ($characteristic, $filter, $event) {
        var _this = this;
        var characteristic = $filter.characteristic;
        var existFilter = this.activeFilters['characteristic'].filter(function (section) { return section.id == characteristic; });
        if (existFilter.length > 0) {
            this.activeFilters['characteristic'].forEach(function (filter, key) {
                if (filter.value == $characteristic && filter.id == characteristic) {
                    _this.activeFilters['characteristic'].splice(key, 1);
                    $event.currentTarget.classList.add('selected');
                }
                    ...
                    _this.activeFilters['characteristic'][key].value = $characteristic;
                    $event.currentTarget.classList.remove('selected');
                }
            });
        }
    ...
    ProductsComponent.prototype.changeString = function ($productType, $brand, $mpn) {
    ...
    ProductsComponent.prototype.changeStringBrand = function ($brand) {
    ...
    ProductsComponent.prototype.getAdditional = function () {
        var _this = this;
        this.appService.getAdditional().subscribe(function (data) {
            _this.additional = data;
        });
    };
    ProductsComponent.prototype.getProductTypes = function () {
        var _this = this;
    ...
    ProductsComponent.prototype.getProducts = function () {
        ...
        if (this.busy) {
            this.busy = 0;
            var brandFilters_1 = this.activeFilters['brand'];
            brandFilters_1 = brandFilters_1.join();
            var characteristicsFilters_1 = this.activeFilters['characteristic'];
            characteristicsFilters_1 = JSON.stringify(characteristicsFilters_1);
            this.appService.getProducts(this.nivel1, this.nivel2, brandFilters_1, characteristicsFilters_1).subscribe(function (data) {
                _this.busy = 1;
                if (brandFilters_1 == "" && characteristicsFilters_1 == "[]" && data.length < 1) {
    ...
    ProductsComponent.prototype.getChangeRange = function (id) {
        var _this = this;
        this.activeFilters['characteristic'].forEach(function (filter, key) {
            if (filter.id == id) {
                _this.activeFilters['characteristic'][key].value = _this.activeFilters['range'][id];
            }
        });
        this.getProducts();
        this.closeSideMenu();
    };
    ProductsComponent.prototype.getSectionsProducts = function (nivel1, nivel2) {
        var _this = this;
        this.appService.getSectionsProducts(nivel1, nivel2).subscribe(function (data) {
            _this.sections = data;
        });
    };
    ...
    ProductsComponent.prototype.filters = function (productType) {
        var _this = this;
        ...
        this.appService.getFilters(productType).subscribe(function (data) {
            _this.listFilters = data;
            for (var _i = 0, _a = _this.listFilters; _i < _a.length; _i++) {
                var filter = _a[_i];
                if (filter.type == 2) {
                    _this.activeFilters['characteristic'].push({ id: filter.characteristic, value: filter.valueMax, type: filter.type });
                    _this.activeFilters['range'][filter.characteristic] = filter.valueMax;
                }
            }
        });
    };
    ...
    ProductsComponent.prototype.onSubmitTeLlamamos = function (values) {
        var _this = this;
        if (this.form.valid) {
            this.appService.enviarBusqueda(values, this.textSearch).subscribe(function (data) {
                var res = JSON.parse(JSON.stringify(data));
                _this.searchSend = res.resultado;
            });
            this.form.reset();
        }
        else {
    ...
    ProductsComponent.prototype.openDeliveryTermsDialog = function () {
        var text = 'Los envíos gratuitos que ofrece JarDepot son a la cobertura terrestre normal de las paqueterías con las que tenemos convenio (ODM).<br>' +
            'NO aplica a zonas extendidas (En extra coberturas se le indicará la diferencia a pagar para su consideración).<br>' +
            ...
            'El tiempo de entrega estimado y sujeto a existencias es de 2 a 6 días hábiles, (Mínimo/Máximo) contados a partir de las siguientes ' +
            '24 hrs de que su depósito se ha verificado y de recibir su correo con los datos completos para facturar y enviar su producto.<br><br>';
        var dialogRef = this.dialog.open(_shared_dialog_dialog_component__WEBPACK_IMPORTED_MODULE_9__["DialogComponent"], {
            panelClass: 'generic-dialog',
            direction: 'ltr'
        });
        dialogRef.componentInstance.title = 'Condiciones de envíos Gratis:';
    ...
    ProductsComponent.prototype.getDistributions = function () {
        this.distributions = this.appService.getDistributions();
    };
    ProductsComponent.prototype.closeSideMenu = function () {
        if (!this.sidenavOpen) {
            this.sidenav.close();
        }
    };
...
/* harmony import */ var _ngx_lite_json_ld__WEBPACK_IMPORTED_MODULE_12__ = __webpack_require__(/*! @ngx-lite/json-ld */ "./node_modules/@ngx-lite/json-ld/fesm5/ngx-lite-json-ld.js");
var __decorate = (undefined && undefined.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
