...
let CartComponent = class CartComponent {
    constructor(appService, _cfr, dialog) {
    ...
    ngAfterViewInit() {
        let listProducts = this.appService.Data.cartList;
        this.appService.getProductsCart().subscribe(res => {
            if (res != null) {
                let listProducts = JSON.parse(res[0]);
                this.dataSource = new _angular_material__WEBPACK_IMPORTED_MODULE_3__["MatTableDataSource"](listProducts);
            }
        });
    }
    ngOnInit() {
    ...
    addComponent() {
        const factory = this._cfr.resolveComponentFactory(_shared_paypal_button_paypal_button_component__WEBPACK_IMPORTED_MODULE_2__["PaypalButtonComponent"]);
    ...
    remove(product) {
        const index = this.appService.Data.cartList.findIndex(item => item.id == product.id);
        if (index !== -1) {
